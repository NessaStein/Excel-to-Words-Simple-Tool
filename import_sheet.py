...
for row in ws.rows:
    ...
    if row[0].value is not None:
        if table_total_cost != 0.00:
            ...
            paragraph = document.add_paragraph(u"仓管：                                               经手人：")
        ...
        paragraph = document.add_paragraph(u"广东省微生物研究所实验材料入库单")
        paragraph_format = paragraph.paragraph_format
        paragraph_format.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER

        date_info = dateutil.parser.parse(str(row[0].value)).strftime('%Y-%m-%d')
        detail_info = u"药用真菌组          日期：" + date_info + u"          发票号：" + str(row[7].value)

        paragraph = document.add_paragraph(detail_info)
        paragraph_format = paragraph.paragraph_format
        paragraph_format.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER

        current_table = document.add_table(rows=1, cols=6)
...
document.save('import_sheet.docx')
